...
const MENU_ADMIN = [
  { label: "Vue d'ensemble", href: "/admin", icon: "squares-four", perm: null },
  { label: "Utilisateurs", href: "/admin/utilisateurs", icon: "users", perm: "utilisateurs" },
  { label: "Validation créateurs", href: "/admin/createurs", icon: "seal-check", perm: "createurs" },
  { label: "Catalogue VOD", href: "/admin/catalogue", icon: "film-slate", perm: "catalogue" },
  ...
  { label: "Playlist VTC", href: "/admin/vtc", icon: "playlist", perm: "vtc" },
...
const NAV_FUTUR = [
  { label: "Lives (modération)", icon: "broadcast" },
  { label: "DOOH géociblé", icon: "map-pin" },
];
    ...
    <div className="min-h-screen flex">
      <aside className="hidden md:flex flex-col w-72 border-r border-outline-variant/10 bg-surface px-6 py-8 shrink-0 fixed left-0 top-0 h-screen z-40">
        <div className="mb-8 mt-2 flex items-center gap-4">
          <div className="w-12 h-12 rounded-full bg-primary-container/20 border-2 border-outline-variant/40 flex items-center justify-center">
        ...
        <nav className="flex-1 flex flex-col gap-1 mt-4 overflow-y-auto">
          {MENU_ADMIN.filter((item) => !item.perm || permissions.includes(item.perm)).map((item) => {
            ...
            return (
              ...
              >
                <i className={`ph-duotone ph-${item.icon} inline-block`} style={{ fontSize: 18 }} />
          ...
          >
            <i className="ph-duotone ph-sign-out" style={{ fontSize: 18 }} />
      ...
      <div className="flex-1 md:ml-72">
        {permRefusee ? (
          <main className="px-6 md:px-12 py-16 text-center">
            <i className="ph-duotone ph-shield text-outline mx-auto mb-4" style={{ fontSize: 48 }} />
